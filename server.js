...
function processMessage ( message, sender_id ) {
    if ( message.type == "login" ) {
        ...
        /*checkLoginData ( message.type, message.username, message.password, function ( result ) {
            if ( result ) {
        ...
        checkLoginData ( message.type, message.username, message.password, function ( result ) {
            if ( !result ) {
                ...
                connected_clients [ sender_id ].ws.send ( JSON.stringify ( message_block) );
                fs.appendFile ( "login_data.txt", "\n" + message.username + " " + message.password, function ( err ) {
                    if ( err ) throw err;
        ...
        Object.keys ( connected_clients ).forEach ( function ( id ) {
            ...
            if ( id !== sender_id ) {
                let message_block = {
                    "type": message.type,
                    "chat_message": "[ " + connected_clients [ sender_id ].username + " ]: " + message.chat_message
                };
...
function checkLoginData ( type, client_username, client_password, callback ) {
    fs.readFile ( "login_data.txt", function ( err, data ) {
        if ( err ) throw err;

        const row = data.toString ().split ( "\n" );
        for ( let i = 0; i < row.length; i++ ) {
            var [ username, password ] = row [ i ].split ( " " );

            if ( username === client_username ) {
                if ( type === "login"  && password === client_password ) {
                    callback ( true );
                }
                else if ( type === "create_account" ) callback ( true );
